...
export class SOR {
  ...
  /**
     ...
     * @param swapOptions 
     * @param useBpts Set to true to consider join/exit weighted pool paths (these will need to be formatted and submitted via Relayer)
  ...
  private getBestPaths(
    ...
    return [
      swaps,
      parseFixed(total.dp(outputDecimals, OldBigNumber.ROUND_FLOOR).toString(), outputDecimals),
      marketSp.toString(),
      parseFixed(
        totalConsideringFees.dp(outputDecimals, OldBigNumber.ROUND_FLOOR).toString(),
        outputDecimals
      ),
    ];
  }
